...
//        plotDataModel!.appendData(dataPoint: [dataPoint])
        ...
        //g_vec.append(b_vec)
        let thisHamilt = myBands.Hamiltonian(band: selectedBand, waveVec_k: myk.kGenerator(), states: 3)
...
//        plotDataModel.changingPlotParameters.title = "Band Structures"
        ...
        let xpoints:[Double] = myk.kGenerator()
        print(xpoints)
        for i in stride(from: 0, to: flatArray.count, by: 1){
            
            let dataPoint: plotDataType = [.X: xpoints[i], .Y : flatArray[i] ]
